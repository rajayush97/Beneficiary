...
const CheckEligibilityScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        const response = await fetch('https://sbwtest.gov.tt/wrapper/api/eligibility/questions/1');
  ...
  const handleCheckEligibility = () => {
    ...
    if (allCorrectAnswered && !anyIncorrectAnswered) {
      navigation.navigate("EligibleScreen"); // Navigate to Eligible screen
    } else {
      navigation.navigate("NotEligibleScreen"); // Navigate to Not Eligible screen
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.staticContent}>
        <Text style={styles.title}>Disaster Relief Grant</Text>
